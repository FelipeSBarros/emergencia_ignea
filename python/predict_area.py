...
def pred_new_coords(lat, lon, d, wind_deg):
    ...
    lon2 = lon1 + math.atan2(
        math.sin(wind_deg) * math.sin(d / R) * math.cos(lat1),
...
def pred_incendio(weather_data):
    ...
    retorna
    """
    logger = logging.getLogger("pred_incendios")
    ...
    # weather_data = data
    for index, hora in enumerate(
        weather_data.get("hourly")[0:24]
    ):  # 24 horas de previsao
        ...
        weather_df = pd.concat(
            [
                ...
                pd.DataFrame(
                    [
                        {
                            "fecha": datetime.utcfromtimestamp(hora.get("dt")).strftime(
                                "%Y-%m-%d"
                            ),
                            "hora": datetime.utcfromtimestamp(hora.get("dt")).strftime(
                                "%H:%M:%S"
                            ),
    ...
    logger.info("Building GDF with predicted fire displacement")
    weather_gdf = gpd.GeoDataFrame(
        weather_df,
        geometry=gpd.points_from_xy(
            weather_df["lon"], weather_df["lat"], crs="EPSG:4326"
        ),
    )
    logger.info("Estimating and returning buffer area of fire displacement")
    weather_gdf["geometry_buffer"] = (
        weather_gdf.to_crs("EPSG:5349")
        .buffer(distance=weather_gdf["viento_vel"].astype(float), resolution=16)
        .to_crs("EPSG:4326")
    )
...
def predict_area():
    logger = logging.getLogger("predict_area")
    incendios_misiones_path = "./datos/"
    if path.exists(incendios_misiones_path):
        try:
            ...
            logger.warning(
                "incendios_misiones.geojson no existente. Probablemente no haya incendios en Misiones"
            )
        else:
            logger.info("Misiones con incencios activos")
            incendios_pred_df = incendios_df.copy()
            incendios_pred_df["buffer_pred"] = None
            logger.info(
                "Getting environmental forecast and running fire displacement prediction"
            )
